...
from posts.posts_schemas import CreatePostForm, EditPostForm, PostView, UserView, CommentView
...
async def get_post_by_id(
    ...
    if user is None:
        ...
        result = db.query(Posts).options(joinedload("owner")).options(joinedload("comments")).get(id)
...
async def get_all_users_posts(
    ...
    return [
        ...
        for result in db.query(Posts).filter(Posts.owner_id == user.get("id"))
        .options(joinedload("owner"))
        .options(joinedload("comments"))
        .all()
    ]
...
async def edit_post(
    ...
    post_model = db.query(Posts).filter(Posts.id == id).filter(Posts.owner_id == user.get("id")).first()
    post_model.content = post.content
